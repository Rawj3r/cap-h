    ...
    class UserModel{
        ...
        */
        ...
        public static function getPublicProfilesOfAllUsers(){
            ...
            $sql = "SELECT * FROM users JOIN person_information ON(users.user_id = person_information.user_id)";
            ...
            foreach ($query->fetchAll() as $user) {
                ...
                $all_users_profiles[$user->user_id]->second_name = $user->second_name;
                $all_users_profiles[$user->user_id]->user_email = $user->user_email;
                $all_users_profiles[$user->user_id]->phone = $user->phone;
                $all_users_profiles[$user->user_id]->person_number = $user->person_number;
                $all_users_profiles[$user->user_id]->role = $user->role;
                $all_users_profiles[$user->user_id]->is_admin = $user->is_admin;
                $all_users_profiles[$user->user_id]->date_hired = $user->date_hired;
        ...
        */
        ...
        public static function getPublicProfileOfUser($user_id){
            ...
            $sql = "SELECT * FROM users JOIN person_information ON(person_information.user_id = users.user_id) WHERE person_information.user_id = :user_id LIMIT 1";
            $query = $database->prepare($sql);
        ...
        public static function getProfileOfUser(){

            $database = DatabaseFactory::getFactory()->getConnection();

            $sql = "SELECT * FROM users JOIN person_information ON(users.user_id = person_information.user_id) WHERE person_information.user_id = :user_id LIMIT 1";
            $query = $database->prepare($sql);
        ...
        */
        ...
        /**
        *   Checks if an ID number is already taken
        *
        *   @param $identity string username
        *
        *   @return bool
        */

        public static function doesIDAlreadyExist($sa_passport){

            $database = DatabaseFactory::getFactory()->getConnection();

            $query = $database->prepare("SELECT sa_passport FROM person_information WHERE sa_passport = :sa_passport LIMIT 1");
            $query->execute(array(':sa_passport' => $sa_passport));
            if ($query->rowCount() == 0) {
                return false;
            }
            return true;
        ...
        /**
        *   Checks if a TAX number is already taken
        *
        *   @param $tax_number string username
        *
        *   @return bool
        */

        public static function doesTAXAlreadyExist($tax_number){

            $database = DatabaseFactory::getFactory()->getConnection();

            $query = $database->prepare("SELECT tax_number FROM person_information WHERE tax_number = :tax_number LIMIT 1");
            $query->execute(array(':tax_number' => $tax_number));
            if ($query->rowCount() == 0) {
                return false;
            }
            return true;
